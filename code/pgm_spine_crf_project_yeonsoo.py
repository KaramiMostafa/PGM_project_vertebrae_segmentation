...
def _evaluate_densecrf(loader: DataLoader,
                       unet: nn.Module,
                       device: torch.device,
                       n_iters: int,
                       sxy_gauss: float,
                       ...
                       sxy_bi: float,
                       srgb_bi: float,
                       compat_bi: float) -> Dict[str, float]:
    """
    Internal helper for evaluating DenseCRF hyperparameters.  It iterates over
    a provided DataLoader (batch size must be 1) and computes evaluation
    metrics for the given DenseCRF configuration.  Images are not saved to
    disk; metrics are aggregated and returned.
    """
    unet.eval()
    agg = {"dice": [], "iou": [], "precision": [], "recall": [], "hd95": [], "assd": [], "bf1": []}
    with torch.no_grad():
        for batch in loader:
            ...
            msk = batch["mask"].numpy()      # (1,1,H,W)
            # Predict probabilities from U-Net
            logits = unet(img)               # (1,2,H,W)
            prob = torch.softmax(logits, dim=1)[:, 1:2, :, :].cpu().numpy()[0, 0]  # (H,W)
            # approximate guidance image in [0,1]
            img_np = img.cpu().numpy()[0, 0]
            img_01 = (img_np - img_np.min()) / (img_np.max() - img_np.min() + 1e-6)
            # Apply DenseCRF with specified parameters
            p_ref = apply_densecrf(img_01, prob, n_iters=n_iters,
                                   sxy_gaussian=sxy_gauss,
                                   compat_gaussian=compat_gauss,
                                   sxy_bilateral=sxy_bi,
                                   srgb_bilateral=srgb_bi,
                                   compat_bilateral=compat_bi)
            pred_mask = (p_ref >= 0.5).astype(np.uint8)
            gt = msk[0, 0].astype(np.uint8)
            metrics = compute_metrics(pred_mask, gt)
            for k, v in metrics.items():
...
def _evaluate_crfrnn(loader: DataLoader, ckpt_path: str, base: int, device: torch.device, n_iters: int) -> Dict[str, float]:
    """
    Internal helper for evaluating a CRF‐as‐RNN model with a specified
    number of mean‐field iterations.  It loads the network from the
    checkpoint but overrides the iteration count when constructing the
    model.  Metrics are computed on the provided loader.
    """
    # Load the checkpoint state dict
    state = torch.load(ckpt_path, map_location=device)
    # Build model with requested iteration count
    model = UNetWithCRF(in_ch=1, base=base, n_iters=n_iters).to(device)
    model.load_state_dict(state["model"])
    model.eval()
    agg = {"dice": [], "iou": [], "precision": [], "recall": [], "hd95": [], "assd": [], "bf1": []}
    with torch.no_grad():
        for batch in loader:
            ...
            log_probs = model(img)  # (B,2,H,W)
            prob = torch.softmax(log_probs, dim=1)[:, 1:2, :, :].cpu().numpy()
            for b in range(prob.shape[0]):
                p = prob[b, 0]
                gt = msk[b, 0].astype(np.uint8)
                pred_mask = (p >= 0.5).astype(np.uint8)
                metrics = compute_metrics(pred_mask, gt)
                for k, v in metrics.items():
                    agg[k].append(v)
...
def densecrf_grid_search(args):
    """
    Perform a small grid search over DenseCRF hyperparameters.  For each
    combination of the provided spatial Gaussian scales, spatial Gaussian
    compatibilities, bilateral spatial scales, bilateral colour scales, and
    bilateral compatibilities, the validation metrics are computed.  A
    checkpoint of a trained U-Net (or CRF-RNN) must be provided via
    --ckpt; the backbone weights are extracted and used for inference.
    """
    set_seed(args.seed)
    device = torch.device('cuda' if torch.cuda.is_available() and not args.cpu else 'cpu')
    data_root = args.data_root
    ...
    _, val_list, _ = split_train_val_test(files_all, seed=args.seed)
    val_ds = SpineProcessedDataset(data_root, val_list, size=(args.size, args.size), augment=False)
    # Use batch_size=1 for DenseCRF convenience
    val_loader = DataLoader(val_ds, batch_size=1, shuffle=False)
    # Load the U-Net backbone from checkpoint
    assert args.ckpt is not None and os.path.isfile(args.ckpt), "Provide --ckpt for the trained model"
    state = torch.load(args.ckpt, map_location=device)
    unet = UNet(in_ch=1, out_ch=2, base=args.base).to(device)
    unet.load_state_dict(state["model"], strict=False)
    unet.to(device)
    # Run grid search
    print("[DenseCRF Grid] Testing combinations of hyperparameters")
    results = {}
    for sxy_g in args.grid_sxy_gauss:
        for compat_g in args.grid_compat_gauss:
            for sxy_b in args.grid_sxy_bi:
                for srgb_b in args.grid_srgb_bi:
                    for compat_b in args.grid_compat_bi:
                        key = (sxy_g, compat_g, sxy_b, srgb_b, compat_b)
                        metrics = _evaluate_densecrf(
                            loader=val_loader,
                            unet=unet,
                            device=device,
                            n_iters=args.densecrf_iters,
                            sxy_gauss=sxy_g,
                            compat_gauss=compat_g,
                            sxy_bi=sxy_b,
                            srgb_bi=srgb_b,
                            compat_bi=compat_b,
                        )
                        results[key] = metrics
                        print(f"  params={key}: {metrics}")
    # Save results
    try:
        import json
        ensure_dir(args.out_dir)
        with open(os.path.join(args.out_dir, "densecrf_grid_results.json"), "w") as f:
            # Convert tuple keys to strings for JSON compatibility
            json.dump({str(k): v for k, v in results.items()}, f, indent=2)
        print(f"Grid search results saved under {args.out_dir}")
    except Exception:
        pass
...
def build_argparser():
    """
    Construct a command line argument parser for the project.  In addition to the
    basic training and evaluation modes, the parser exposes options for
    performing hyperparameter sweeps.  The iteration sweep can be used to
    empirically study how the number of mean‐field iterations impacts
    performance for both the DenseCRF post–processing and the CRF‐as‐RNN
    network.  The DenseCRF grid search explores a small set of spatial and
    bilateral filter parameters and compatibility weights.  See the README or
    the usage examples in the top of this file for further details.
    """
    p = argparse.ArgumentParser(description="CNN--CRF for Vertebrae Segmentation")
    p.add_argument("--data_root", type=str, default="/home/yec23006/projects/research/KOMP/PGM_project_vertebrae_segmentation/data", help="Path to dataset root with Processed/ and Mask/")
    p.add_argument("--out_dir", type=str, default="outputs", help="Where to save models and predictions")
    # Add new modes for hyperparameter sweeps
    p.add_argument(
        "--mode",
        type=str,
        required=True,
        ...
        help=(
            "What to run: training/evaluation of U-Net or CRF-RNN, DenseCRF post processing, "
    ...
    p.add_argument(
        ...
        type=int,
        default=10,
        help="Maximum number of iterations for sweep. Only used in crf_iter_sweep mode.",
    )
    p.add_argument(
        "--grid_sxy_gauss",
        type=float,
        nargs="*",
        default=[3.0, 5.0],
        help="List of spatial Gaussian scales for DenseCRF grid search.",
    )
    p.add_argument(
        "--grid_compat_gauss",
        type=float,
        nargs="*",
        default=[3.0, 5.0],
        help="List of spatial Gaussian compatibilities for DenseCRF grid search.",
    )
    p.add_argument(
        "--grid_sxy_bi",
        type=float,
        nargs="*",
        default=[30.0, 50.0],
        help="List of bilateral spatial scales for DenseCRF grid search.",
    )
    p.add_argument(
        "--grid_srgb_bi",
        type=float,
        nargs="*",
        default=[5.0],
        help="List of bilateral color scales for DenseCRF grid search.",
    )
    p.add_argument(
        "--grid_compat_bi",
        type=float,
        nargs="*",
        default=[5.0, 10.0],
        help="List of bilateral compatibilities for DenseCRF grid search.",
    )
    return p
...
def main():
    ...
    if args.mode == "train_unet":
        ...
        eval_crfrnn(args)
    elif args.mode == "crf_iter_sweep":
        crf_iteration_sweep(args)
    elif args.mode == "densecrf_grid":
        densecrf_grid_search(args)
    else:
